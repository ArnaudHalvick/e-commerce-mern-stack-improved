...
/**
 * Test script for sending an email with Resend SMTP
 * Run with: node testSMTPEmail.js
 */
async function testSMTPEmail() {
  try {
    ...
    console.log("- SMTP Port:", process.env.RESEND_SMTP_PORT || "465");
    console.log(
      "- From Email:",
    ...
    const testEmailOptions = {
      ...
      html: `
        <h1>Testing Resend Email Service with SMTP</h1>
        <p>This is a test email sent using Resend SMTP integration.</p>
        <p>Configuration:</p>
        <ul>
          <li>SMTP Port: ${process.env.RESEND_SMTP_PORT || "465"}</li>
          <li>From Email: ${process.env.FROM_EMAIL || "onboarding@resend.dev"}</li>
          <li>Date/Time: ${new Date().toLocaleString()}</li>
        </ul>
        <p>If you're seeing this email, the Resend SMTP integration is working correctly!</p>
      `,
    };

    // Send the test email
    const result = await sendEmail(testEmailOptions);
    logger.info("Test email sent successfully with Resend SMTP!");
    logger.info("Result:", result);

    console.log(
      "\x1b[32m%s\x1b[0m",
      "✅ Test email sent successfully with Resend SMTP!"
    );
